...
//fotonya
const fakeimage = fs.readFileSync ('./media/Hanif.jpg')

//Module Export
module.exports = Hanif = async (Hanif, mek, _welkom) => {
try {
...
const botNumber = Hanif.user.jid
const ownerNumber = [`${ownernumber}@s.whatsapp.net`, `${ownernumber2}@s.whatsapp.net`]
const isGroup = from.endsWith('@g.us')
const isQuotedReply = type === 'extendedTextMessage' && content.includes('Message')
let sender = isGroup ? mek.participant : mek.key.remoteJid
let senderr = mek.key.fromMe ? Hanif.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
const groupMetadata = isGroup ? await Hanif.groupMetadata(from) : ''
...
const conts = mek.key.fromMe ? Hanif.user.jid : Hanif.contacts[sender] || { notify: jid.replace(/@.+/, '') }
const pushname = mek.key.fromMe ? Hanif.user.name : conts.notify || conts.vname || conts.name || '-'
...
let bio_nya = await Hanif.getStatus(sender)
try {
...
const reply = (teks) => {
Hanif.sendMessage(from, teks, text, {quoted:mek})
}
const sendMess = (hehe, teks) => {
Hanif.sendMessage(hehe, teks, text)
}
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? Hanif.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : Hanif.sendMessage(from, teks.trim(), extendedText, { quoted: ftrol, contextInfo: { "mentionedJid": memberr } })
}
const costum = (pesan, tipe, target, target2) => {
Hanif.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
}
...
await Hanif.setStatus(`${botname} | ⏲️ Runtime : ${uptimer}`).catch((_) => _);
...
Hanif.sendMessage(to, media, MessageType.sticker,{quoted:mek})
...
Hanif.sendMessage(from, buttonMessages, buttonsMessage, {
quoted: ftrol
})
}
...
jadinya = await Hanif.prepareMessage(from, img, image)
buttonMessagesI = {
...
Hanif.sendMessage(from, buttonMessagesI, buttonsMessage, {
quoted: ftrol,
})
}
async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
const buttonMessages = { locationMessage: { jpegThumbnail: gam1 }, contentText: text1, footerText: desc1, buttons: but, headerType: 6 }
return Hanif.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButDocument = async(id, text1, desc1, media, doc1, but = [], options = {}) => {
kma = doc1
mhan = await Hanif.prepareMessage(from, media, document, kma)
...
Hanif.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await Hanif.prepareMessage(from, kma, video)
const buttonMessages = {
...
+ 'END:VCARD'.trim()
Hanif.sendMessage(from, {displayName: `Ownernya ${botname}`, vcard: vcard2}, contact, 
{ quoted: ftrol, 
})
...
await Hanif.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: mek})
break
case 'self':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner) 
...
anu100 = await Hanif.chats.all()
if (isMedia && !Hanif.message.videoMessage || isQuotedImage) {
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Hanif).replace('quotedM','m')).message.extendedTextMessage.contextInfo : Hanif
bc100 = await Hanif.downloadMediaMessage(encmedia)
for (let _ of anu100) {
Hanif.sendMessage(_.jid, bc100, image, {quoted: ftrol, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
reply('Suksess broadcast')
} else {
for (let _ of anu100) {
Hanif.sendMessage(_.jid, 
...
Hanif.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: mek })
break
case 'setnamabot':{
if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner) 
if (args.length < 1) return reply(`Kirim perintah ${command} nama\n\nContoh : ${command} HanifSih`)
Hanif.updateProfileName(q)
...
case 'infobot':{
try {
let totalchat = await Hanif.chats.all()
let i = []
let giid = []
...
const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = Hanif.user.phone
...
Browser : ${Hanif.browserDescription}`
teks =
`${botname} | Runtime : ${runtime(process.uptime())}\nOwner : ${ownername}`
Hanif.sendMessage(from, { contentText: `${menu}`, footerText: `${teks}`, buttons: [{ buttonId: `${prefix}donasi`, buttonText: { displayText: 'DONASI 💵' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER 🌱' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
} catch (e){
...
case 'sc':
case 'script':
menu = `*Info Base📪*\n📚 Base Made By Hanif Botz\n📩 Link Base : https://youtube.com/channel/UCDbJPJmK_RGB-CPKrLElzEA\n♨️ Sumber Base : Hanif Botz\n\n🍁 Subscribe For More Update`
...
const media = await Hanif.downloadAndSaveMediaMessage(encmedia)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
...
.on('end', function () {
console.log('Finish')
Hanif.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
media = await Hanif.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Gagal, pada saat mengkonversi sticker ke gambar')
buffer = fs.readFileSync(ran)
Hanif.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
fs.unlinkSync(ran)
})
break
...
owgi = await Hanif.downloadMediaMessage(boij)
...
break
case 'ttp':
if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp Hanifka Botz`)
anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
Hanif.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
break
case 'attp':
if (args.length == 0) return reply(`Example: ${prefix + command} Hai`)
buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
Hanif.sendMessage(from, buffer, sticker, { quoted: mek })
break
...
await Hanif.groupRemove(from, [kickya])
reply(`Succses`)
break
case 'linkgc': case 'linkgrup': case 'linkgroup':
if (!isGroup) return reply(mess.only.group)
if (!isBotGroupAdmins) return reply(mess.only.Badmin) 
Hanif.groupInviteCode(from)
...
let media = await Hanif.downloadMediaMessage(encmedia)
Hanif.updateProfilePicture(from, media)
...
buffer = fs.readFileSync(ran)
Hanif.updateProfilePicture(from, buffer)
fs.unlinkSync(ran)
})
} else {
...
Hanif.groupUpdateSubject(from, q)
.then((res) => reply(jsonformat(res)))
.catch((err) => reply(jsonformat(err)))
break
...
if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
if (args.length == 0)  return reply(`Penggunaan ${prefix + command} Alphabot Support`)
Hanif.groupUpdateDescription(from, q)
.then((res) => reply(jsonformat(res)))
.catch((err) => reply(jsonformat(err)))
break
...
Hanif.sendMessage(from, ytmp3, audio, {mimetype:"audio/mp4", quoted:mek})
break
case 'ytmp4':
if (args.length < 1) return reply("Masukkan url youtube")
url = args.join(' ')
...
Hanif.sendMessage(from, ytmp4, video, {caption:`Done kak`, thumbnail:Buffer.alloc(0), quoted:mek})
break
